import type { AnalysisArtifact } from '../steps/03-analyze';
import type { EndpointsArtifact, NodesArtifact, ReadingsArtifact } from '../types';

interface ReportInput {
  endpoints: EndpointsArtifact;
  nodes: NodesArtifact;
  readings: ReadingsArtifact;
  analysis: AnalysisArtifact;
}

export function buildReport({ endpoints, nodes, readings, analysis }: ReportInput): string {
  const sitios = analysis.sitios;
  const fecha = new Date().toISOString().slice(0, 10);

  const sitiosTabla = sitios
    .map((s) => {
      const buffers =
        s.buffers.entradaReal.length +
        s.buffers.entradaInt.length +
        s.buffers.entradaBit.length +
        s.buffers.salidaReal.length +
        s.buffers.salidaInt.length;
      return `| ${s.site} | ${buffers} | ${s.buffers.salidaInt.length + s.buffers.salidaReal.length} | ${s.comunicacion.estado} |`;
    })
    .join('\n');

  const comandosBuffers = [...new Set(analysis.comandosCandidatos.map((s) => s.bufferBrowseName))].sort();

  return `# Informe de Integración OPC UA — PTAP AQUATECH

> ${analysis.arquitecturaDetectada.descripcion}

**Generado:** ${fecha} · **Método:** ingeniería inversa de SOLO LECTURA (Browse/Read)
**Servidor:** ${endpoints.requestedEndpoint} (${endpoints.server.productName ?? 'FactoryTalk Optix'}, estado ${endpoints.server.state})
...
${analysis.arquitecturaDetectada.evidencia.map((e) => `- ${e}`).join('\n')}
...
### Sitios remotos detectados (${sitios.length})
...
**El servidor OPC UA no expone la semántica del proceso.** Medido sobre ${readings.stats.variablesRead} variables:

| Metadato | Tags que lo declaran |
|---|---|
| EngineeringUnits | ${readings.stats.withEngineeringUnits} |
| EURange | ${readings.readings.filter((r) => r.attrs.euRange).length} |
| InstrumentRange | ${readings.readings.filter((r) => r.attrs.instrumentRange).length} |
| Description | ${readings.readings.filter((r) => r.attrs.description).length} |
...
${analysis.fiabilidadMuestreo.conclusion}

Es decir: se capturaron 3 muestras espaciadas 45 s, pero de ${analysis.fiabilidadMuestreo.buffersArray} buffers de array, ${analysis.fiabilidadMuestreo.buffersArrayConRelecturaBad} devolvieron \`BadInternalError\` en la 2.ª y 3.ª lectura. Esto **no** es un fallo de la herramienta: es el driver \`RAEtherNet_IP\` de Optix, que sirve los tags bajo demanda y no sostiene re-lecturas masivas. **Refuerza directamente la decisión de arquitectura**: el refresco de datos en el backend debe hacerse por **Subscription** (el servidor empuja los cambios a su ritmo), nunca por polling agresivo de \`Read\`.
...
El servidor anuncia \`${endpoints.hostnameMismatch.announcedByServer.join(', ')}\` pero se accede por \`${endpoints.hostnameMismatch.external}\`. El adaptador **debe** crear el cliente con \`endpointMustExist: false\` para mantener el socket hacia la IP pública:
...
${nodes.namespaces.map((u, i) => `- \`ns=${i}\` → \`${u}\``).join('\n')}
...
Buffers de salida (canal de comando) detectados: ${comandosBuffers.length ? '\`' + comandosBuffers.join('`, `') + '\`' : '(ninguno)'}.
...
- **Un solo** endpoint: \`${endpoints.requestedEndpoint}\`.
- Los sitios reales son: ${sitios.map((s) => s.site).join(', ')}.
